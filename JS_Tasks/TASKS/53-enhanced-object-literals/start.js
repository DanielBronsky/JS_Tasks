...
/**
 * Потому что он создается внутри замыкания, которое включает в себя параметры функции. Функция "photosGallery" возвращает объект, содержащий метод "publishInfo", и этот метод сохраняет ссылку на замыкание, в котором он был создан. Таким образом, он имеет доступ ко всем переменным и параметрам, которые были доступны в этом замыкании.
 */
